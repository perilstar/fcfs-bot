...
class RemoveAdminRoleCommand extends discord_akairo_1.Command {
    constructor() {
    ...
    exec(message, args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!adminRoles.includes(args.role.id)) {
                sendmessage_1.default(message.channel, `Error: ${args.role.name} is not set as bot admin!`);
                return;
            }
            const index = server.adminRoles.indexOf(args.role.id);
            server.adminRoles.splice(index, 1);
            ds.saveServer(server.id);
            sendmessage_1.default(message.channel, `Successfully removed role ${args.role.name} as bot admin!`);
        });
    }
}
exports.default = RemoveAdminRoleCommand;
