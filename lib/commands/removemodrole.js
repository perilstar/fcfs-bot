...
class RemoveModRoleCommand extends discord_akairo_1.Command {
    constructor() {
        super('removemodrole', {
    ...
    exec(message, args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!modRoles.includes(args.role.id)) {
                sendmessage_1.default(message.channel, `Error: ${args.role.name} is not set as bot mod!`);
                return;
            }
            const index = server.modRoles.indexOf(args.role.id);
            server.modRoles.splice(index, 1);
            ds.saveServer(server.id);
            sendmessage_1.default(message.channel, `Successfully removed role ${args.role.name} as bot mod!`);
        });
    }
}
exports.default = RemoveModRoleCommand;
