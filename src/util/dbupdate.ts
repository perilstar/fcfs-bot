...
async function v1to2(db: Database) {
  ...
  const results = await db.all('SELECT guild_id, mod_roles FROM monitor');
...
async function v2to3(db: Database) {
...
export default async function dbupdate(db: Database) {
