...
export default class AddHelperRoleCommand extends Command {
  constructor() {
    super('addhelperrole', {
  ...
  async exec(message: Message, args: any) {
    ...
    if (helperRoles.length >= Constants.AddedRoles.MAX) {
      sendmessage(
        ...
        `Error: You can not add more than ${Constants.AddedRoles.MAX} roles as bot helper!`,
      );
      return;
    }

    if (helperRoles.includes(args.role.id)) {
      sendmessage(<TextChannel> message.channel, `Error: ${args.role.name} is already set as bot helper!`);
      return;
    }
    ...
    sendmessage(<TextChannel> message.channel, `Successfully added role ${args.role.name} as a bot helper!`);
  }
}
